...
from robotcode.core.utils.dataclasses import as_dict, as_json, from_json
...
def _extract_force_tags_from_path(path: Path) -> List[str]:
    ...
    force_tag_tokens = {
        token_type
        for token_type in (getattr(Token, "FORCE_TAGS", None), getattr(Token, "TEST_TAGS", None))
...
def _extract_fast_items_from_path(path: Path) -> Iterable[FastExtractedItem]:
    current_section: Optional[str] = None
    token_source = _get_token_source_for_path(path)
    ...
    for row_tokens in _iter_statements(token_source):
        ...
        if first.type == Token.TESTCASE_HEADER:
            ...
            continue
        if first.type == Token.TASK_HEADER:
            if current_item is not None:
                yield current_item
                current_item = None
        ...
        if first.type == Token.TAGS:
            current_item.tags.extend(str(t).strip() for t in row_tokens[1:] if str(t).strip())
            continue
...
def _build_fast_discovery_result(
    ...
    for index, file_path in enumerate(files, start=1):
        ...
        extracted_count = 0
        for extracted_item in _extract_fast_items_from_path(file_path):
            extracted_count += 1
            combined_tags = list(dict.fromkeys([*inherited_force_tags, *extracted_item.tags]))
